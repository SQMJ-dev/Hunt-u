...
@export var buff_icons : Dictionary = {}
...
func play_attack_anim():
	...
	tween.tween_property($Sprite2D, "position", origin + Vector2(0, -20), 0.10)
	tween.tween_property($Sprite2D, "position", origin,                   0.10)
	await tween.finished
	
func refresh_buff_icons():
	_refresh_icon($BuffDisplay/AtkIcon, "ATK", buff_stages["ATK"])
	_refresh_icon($BuffDisplay/DefIcon, "DEF", buff_stages["DEF"])
	_refresh_icon($BuffDisplay/AgiIcon, "ACC", buff_stages["AGI"])

func _refresh_icon(icon : Sprite2D, stat : String, stage : int):
	if stage == 0:
		icon.hide()
		return
	icon.show()
	var direction = "UP" if stage > 0 else "down"
	var level     = "2" if abs(stage) == 2 else ""
	var path      = "res://assets/" + stat + direction + level + ".png"
